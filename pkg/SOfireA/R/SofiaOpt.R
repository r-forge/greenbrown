SofiaOpt <- structure(function(
	...
	##description<<
	## The function fits a SOFIA model to observations by estimating model parameters using \code{\link{Sofia}} genetic optimization. 
	...
	par.init = NULL,
	### matrix of inital parameters for optimization
	...
	cost = NULL,
	### a function to compute the cost, if NULL SSE (sum of squared error) is used
   ...
   if (restart == 1) {
      ...
      if (length(files.result) > 0) {
         message("FitSofia: read from previous results for restart")
      ...
      if (nodes > 1) {
         ...
         require(parallel)
	      cluster <- makeCluster(nodes)
		   ...
		   clusterExport(cluster, c("Sofia", "SofiaPar", "Logistic", "x", "area", "obs", "unc", "per.group", ".error", "cost", "SSE", "ObjFct", "ReadSofiaOpt", "path", "par.prior"), envir=environment())
		   #clusterExport(cluster, c("Sofia", "SofiaPar", "Logistic", ".error", "cost", "SSE", "ObjFct", "ReadSofiaOpt", "path", "par.prior"), envir=environment())

		   message(paste("FitSofia: Finished preparing cluster nodes for parallel computing.", Sys.time()))
      } 
      
      # perform genetic optimization
      require(rgenoud)
      message(paste("FitSofia: Starting genoud optimization.", Sys.time()))
   ...
   message(paste("FitSofia: Prepare results.", Sys.time()))
...
#   save(result, file=tempfile("genoud_", tmpdir=path, "_result.RData"))
   ...
   plot(sf$data$obs, sf$data$y, ylab="predicted", xlab="observed", main=sf$eq, pch=16, col=rgb(0.2, 0.2, 0.2, 0.2), cex=0.5)  
   abline(0,1)
   legend("topleft", text(result[[best]]$obj, sep="\n"), bty="n", text.col="red") 
   boxplot(sf$data$obs, sf$data$y, ylab="", names=c("Obs", "Sim"))
   dev.off()
